...
export const DropDown = forwardRef((props: DropDownProps, _) => {
  ...
  useEffect(() => {
    if (onCheckType(onChangeItem, 'function')) {
      if (Array.isArray(selectedValue)) {
        onChangeItem(
          selectedValue,
          data.reduce((prev, current, _value, arr) => {
  ...
  const contentModalStyle = useMemo<StyleProp<ViewStyle>>(
    () => [
      ...
      dropDownStyle,
      {
        width: wrapMeasured.value.width,
        left: wrapMeasured.value.x,
      },
      isRenderOnBottom
        ? { top: wrapMeasured.value.y + wrapMeasured.value.height / 2 }
        : {
    ...
    <>
            ...
            {showArrow &&
              (renderArrow ? (
                ...
                <Animated.View style={[arrowStyle]}>
                  <Icon icon={'arrow_down'} colorTheme="white" />
                </Animated.View>
              ))}
          </View>
        </TouchableOpacity>
      ...
      >
        <View onLayout={onLayoutDrop} style={contentModalStyle}>
          <TouchableOpacity
            style={{
              height: wrapMeasured.value.height / 2,
            }}
            onPress={() => {
              setIsVisible(false);
            }}
