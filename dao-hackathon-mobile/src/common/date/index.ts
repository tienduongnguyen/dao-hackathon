...
export const dateBetweenRange = ({
  ...
  const mCheckDate = moment(checkDate, format);
  const mStartDate = moment(startDate, format);
  const mEndDate = moment(endDate, format);
  return mCheckDate.isBetween(mStartDate, mEndDate);
};
...
export function enumerateDaysBetweenDates(
  ...
  const currDate = moment(startDate, dateFormat).startOf('day');
  const lastDate = moment(endDate, dateFormat).startOf('day');
