...
import { getListProposal } from "redux/proposalSlice"
...
export default function CreateProposalModal(props: Props){
    ...
    const handleResetTextArea = () => {
        const reset = document.querySelector('.text-area-child') as HTMLTextAreaElement
        reset.value = ""
    }

    const handleSubmit = async () => {
        const proposalName = document.querySelector('#ten-proposal') as HTMLInputElement
        const res_name = proposalName.value
        ...
        const description = document.querySelector('.desription') as HTMLTextAreaElement
        const res_des = description.value
        if (!window.ethereum) {
        ...
        const contracts = new ethers.Contract(config.MesmeGovernor.address, config.MesmeGovernor.abi, signer);
        const targets = ["0x0000000000000000000000000000000000000000"]
        const values = [0]
        const calldatas = ["0x00"]
        const des = res_des
        // call contract create proposal
        await contracts.propose(targets, values, calldatas, des)
        const proposeHash = await daoService.getHashProposal(targets, values, calldatas, des)
    
        const proposalId = await proposeHash.data.data.proposalId
        let address = "";
        ...
        // call API save proposal to database
        const createProposal = await proposalService.createProposal(proposalId, res_name, address, timestampStart, timestampEnd,res_des);
        const updateAll = await proposalService.updateAllProposals();
        dispatch(getListProposal(updateAll.data.data))
        handleClose()
    }
                        ...
                        <div className="name-proposal">
                            ...
                            <div className="content2">
                                <div className="content2-title">
                                    ...
                                    <div className="text-input">
                                        <input type="text" defaultValue="GenericProposal" readOnly></input>
                                    </div>
                                </div>
                                <div className="date">
                            ...
                            <div className="text-area">
                                ...
                                <div className="text-area-input">
                                    <textarea className="text-area-child desription" onClick={handleResetTextArea}></textarea>
                                </div>
                            </div>
